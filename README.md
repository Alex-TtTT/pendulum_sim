...
系统是长度为 \\(L\\)、质量 \\(m\\) 的摆，考虑重力、气动(或阻力)项与**受控水平推力**（在小角窗内、随角速度方向施加）以及**底部反向刹车**（在靠近竖直附近按角度与角速度自适应地给反向力）。

动力学采用非线性方程：


\\[
\\theta'' \\,=\\, -\\frac{g}{L}\\sin\\theta \\, + \\, \\frac{F_h}{mL}\\cos\\theta \\, + \\, \\frac{\\kappa_{\\text{eff}} g}{L}\\cos\\theta \\, - \\, \\frac{c_t}{m}\\,\\omega,
\\]

其中 \\(\\kappa_{\\text{eff}} = \\kappa_0 \\sin\\theta\\,\\text{sign}(\\omega)\\)。
